...
def main():
    ...
    # Return a list of decoded ascii messages
    cipherTextsDecoded = map(methodcaller("decode", "hex"), cipherTexts)
    # Crib dragging
    ...
    xored = strxor(cipherTextsDecoded[xorTextNum1], cipherTextsDecoded[xorTextNum2])
    
    length = len(xored)
    
    for i in range(length):
            substring = xored[i:]
            cribDraggedText = strxor(substring,crib)
    key = strxor(cipherTextsDecoded[xorTextNum1], crib)
    print("\n\nDecrypt all messages using the key:")
    for j, cipher in enumerate(cipherTextsDecoded) :
        print("%s\n" % (strxor(cipher, key)))


	
	
# strxor function is taken from https://www.coursera.org/learn/crypto/quiz/KZ9js/week-1-programming-assignment-optional
